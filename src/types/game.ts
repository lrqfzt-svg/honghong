...
// 获取对应性别的语音列表
export const getVoicesByGender = (gender: Gender): VoiceConfig[] => {
  return VOICE_CONFIGS.filter((v) => v.gender === gender);
};

// 根据语音类型获取 Speaker
export const getSpeakerByVoiceType = (voiceType: VoiceType): string => {
  const config = VOICE_CONFIGS.find((v) => v.voiceType === voiceType);
